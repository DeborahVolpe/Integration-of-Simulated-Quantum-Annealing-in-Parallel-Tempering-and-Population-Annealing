...
class PathIntegralMonteCarloAlgorithmTemperingPopulationv2:
    ...
    def Solve(self, numberOfIterationIn, numberOfTimeIn):
        ...
        for f in range(self.NumberOfTime):
            ...
            for i in range(self.NumberOfIteration):
                ...
                # Gamma of the last tempering replicas
                LastGamma = self.Gamma[self.s[self.NumberOfSystemTempering-1]] - self.Gamma0*(1/(self.NumberOfIteration+1))
                
                # Saturation in case of negative Last Gamma
                if self.Gamma[self.s[self.NumberOfSystemTempering-1]] <= self.Gamma0*(1-self.NumberOfIteration/(self.NumberOfIteration+1)):
                    LastGamma = self.Gamma0*(1-self.NumberOfIteration/(self.NumberOfIteration+1))
                ...
                if i != self.NumberOfIteration-1:
                    if m > 1000:
                        Q += math.exp((0.5*math.log(((math.sqrt(self.Gamma[self.s[self.NumberOfSystemTempering-1]]**2 +1)+1)/self.Gamma[self.s[self.NumberOfSystemTempering-1]])**2) - 0.5*math.log(((math.sqrt((LastGamma**2 +1)+1)/LastGamma))**2)) * (BetterValueOfEachSystemTempering[self.NumberOfSystemTempering-1]/m))/(self.NumberOfSystemPopulation+1)
                    else: 
                        Q += math.exp((0.5*math.log(((math.sqrt(self.Gamma[self.s[self.NumberOfSystemTempering-1]]**2 +1)+1)/self.Gamma[self.s[self.NumberOfSystemTempering-1]])**2) - 0.5*math.log(((math.sqrt((LastGamma**2 +1)+1)/LastGamma))**2)) * BetterValueOfEachSystemTempering[self.NumberOfSystemTempering-1])/(self.NumberOfSystemPopulation+1)
                        ...
                        if m > 1000:
                            Q += math.exp((0.5*math.log(((math.sqrt(GammaP**2 +1)+1)/GammaP)**2)
                                       - 0.5*math.log(((math.sqrt((self.Gamma0*(1-(i+1)/(self.NumberOfIteration+1)))**2 +1)+1)
                                                       /(self.Gamma0*(1- (i+1)/(self.NumberOfIteration+1))))**2)) * (BetterValueOfEachSystemPopulation[System]/m))/(self.NumberOfSystemPopulation+1) 
                        else:
                            Q += math.exp((0.5*math.log(((math.sqrt(GammaP**2 +1)+1)/GammaP)**2)
                                       - 0.5*math.log(((math.sqrt((self.Gamma0*(1-(i+1)/(self.NumberOfIteration+1)))**2 +1)+1)
                                                       /(self.Gamma0*(1- (i+1)/(self.NumberOfIteration+1))))**2)) * BetterValueOfEachSystemPopulation[System])/(self.NumberOfSystemPopulation+1)
                if Q != 0:
                    # Number of copy defined
                    systemCopy = self.NumberOfSystemTempering-2
                    ...
                    if m > 1000:
                        # Evaluate the number of last parallel tempering replicas copies to mantain
                        Nmean = (1/Q) * math.exp((0.5 * math.log(((math.sqrt(self.Gamma[self.s[self.NumberOfSystemTempering-1]] ** 2 + 1) + 1) / self.Gamma[self.s[self.NumberOfSystemTempering-1]])**2) - 0.5 * math.log((math.sqrt((LastGamma**2 + 1) + 1) / (LastGamma))**2))*(BetterValueOfEachSystemTempering[self.NumberOfSystemTempering-1]/m))
                    else:
                        # Evaluate the number of last parallel tempering replicas copies to mantain
                        Nmean = (1/Q) * math.exp((0.5 * math.log(((math.sqrt(self.Gamma[self.s[self.NumberOfSystemTempering-1]] ** 2 + 1) + 1) / self.Gamma[self.s[self.NumberOfSystemTempering-1]])**2) - 0.5 * math.log((math.sqrt((LastGamma**2 + 1) + 1) / (LastGamma))**2))*BetterValueOfEachSystemTempering[self.NumberOfSystemTempering-1])
                    Number = np.random.poisson(Nmean, size=1)[0]
                    for p in range(Number):
                        if systemCopy < (self.NumberOfSystemPopulation + self.NumberOfSystemTempering):
                            temp[systemCopy] = SpinsMatrix[self.NumberOfSystemTempering-1].copy()
                            for t in range(self.NumberOfReplica):
                                tempEnergyOfEachSystemReplica.itemset((systemCopy, t), EnergyOfEachSystemReplica.item(((self.NumberOfSystemTempering-1), t)))
                            systemCopy += 1
                ...
                JplusP = TempP/2*math.log(coth(GammaP/(self.NumberOfReplica*TempP)))
                # Correlation factor between replica
                #Jplus = self.T / 2 * math.log(coth(GammaP / (self.NumberOfReplica * self.T)))
    ...
    def Plot(self, FileName="", Last=True, Latex=False, Save=False):
        if Latex == True:
            ...
            n, bins, patches = plt.hist(self.finalValues, bins=100, cumulative=True, histtype='step', linewidth=2,label=r'\textit{Quantum Parallel Tempering - Population Annealing v2}')#cumulative=True, histtype='step',
            ...
            n, bins, patches = plt.hist(self.finalValues, bins=100, cumulative=True, histtype='step', linewidth=2,label="Quantum Parallel Tempering - Population Annealing v2")#cumulative=True, histtype='step',
    ...
    def WriteResultsInAFile(self, nomeFileReport,  **kwargs):
        ...
        ReportFile.write("----------Report of simulation of Quantum Parallel Tempering - Population Annealing v2-----------\n")
